...
from app.database.database import (
    ...
    obtener_etiquetas_cuentas_pedido, obtener_comensales_mesa,
    registrar_cliente,
)
...
function showAccount(){const a=state.accounts[mesa.value];if(!a){accountInfo.className='account';accountInfo.textContent='';return}accountInfo.className='account open';accountInfo.replaceChildren();const title=document.createElement('strong'),detail=document.createElement('span');title.textContent=`Saldo pendiente · ${money(a.total)}`;const cuentas=(a.comensales||[]).map(x=>`Comensal ${x.numero}: ${x.pagada?'PAGADO':money(x.total)}`).join(' · ');detail.textContent=cuentas||`${a.pedidos} pedido(s) pendiente(s). Lo nuevo se agregará a esta cuenta.`;accountInfo.append(title,detail)}
