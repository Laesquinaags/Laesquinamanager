...
.account{display:none;margin:-3px 0 12px;padding:11px 13px;border-radius:9px;background:#fff4c7;border:1px solid #e3c14f;color:#56470d;font-size:14px;font-weight:bold}.account.open{display:block}.account strong{display:block;font-size:17px;margin-bottom:3px}.diner-tabs{display:flex;gap:6px;overflow:auto;margin:9px 0}.diner-tabs button{border:1px solid #b99b2e;background:white;border-radius:8px;padding:8px;white-space:nowrap;font-weight:bold}.diner-tabs button.active{background:#3b332d;color:white}.ordered{background:white;border-radius:8px;padding:8px;margin-top:7px}.ordered div{display:flex;justify-content:space-between;border-bottom:1px solid #eee;padding:5px}.paid{color:#278447}
...
function showAccount(){const a=state.accounts[mesa.value];if(!a){accountInfo.className='account';accountInfo.textContent='';return}accountInfo.className='account open';accountInfo.replaceChildren();const title=document.createElement('strong'),detail=document.createElement('span'),tabs=document.createElement('div'),ordered=document.createElement('div');title.textContent=`Cuenta abierta · ${money(a.total)}`;detail.textContent='Selecciona un comensal para ver lo que pidió o agregarle productos.';tabs.className='diner-tabs';ordered.className='ordered';const diners=a.comensales||[];const max=Math.max(1,...diners.map(c=>c.numero));if(!Object.keys(state.cart).length){numeroCuentas.value=String(max);rebuildAccounts()}const selected=Number(cuentaActual.value==='shared'?1:cuentaActual.value);const renderExisting=n=>{ordered.replaceChildren();const c=diners.find(x=>x.numero===n);if(!c){ordered.textContent='Este comensal no tiene productos pendientes.';return}const head=document.createElement('strong');head.textContent=`Comensal ${n} · ${c.estado} · ${money(c.total)}`;ordered.appendChild(head);c.productos.forEach(p=>{const row=document.createElement('div'),name=document.createElement('span'),price=document.createElement('span');name.textContent=p[0];price.textContent=money(p[1]);row.append(name,price);ordered.appendChild(row)})};diners.forEach(c=>{const b=document.createElement('button');b.textContent=`C${c.numero} · ${money(c.total)}`;b.classList.toggle('active',c.numero===selected);b.onclick=()=>{selectDiner(String(c.numero));showAccount()};tabs.appendChild(b)});renderExisting(selected);accountInfo.append(title,detail,tabs,ordered)}
